...
typedef std::minstd_rand hcnet_default_random_engine;

extern hcnet_default_random_engine gRandomEngine;

template <typename T>
T
rand_uniform(T lo, T hi)
{
    return hcnet::uniform_int_distribution<T>(lo, hi)(gRandomEngine);
}
...
#ifdef BUILD_TESTS
// This function should be called any time you need to reset hcnet-core's
